...
export const showToolTip = function(pGanttChartObj, e, pContents, pWidth, pTimer) {
    const vTtDivId = pGanttChartObj.getDivId() + 'JSGanttToolTip';
    const vMaxW = 500;
    const vMaxAlpha = 100;
    const vShowing = pContents.id;
...
export const syncScroll = function(elements, attrName) {
    const syncFlags = new Map(elements.map(e => [e, false]));
