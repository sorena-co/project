...
import { getOffset, getScrollbarWidth } from './utils/general_utils';
...
export const GanttChart = function(pDiv, pFormat) {
    ...
    this.vMinGpLen = 8;
    this.vScrollTo = '';
    this.vHourColWidth = 36;
    this.vDayColWidth = 36;
    this.vWeekColWidth = 64;
    this.vMonthColWidth = 64;
    this.vQuarterColWidth = 36;
    this.vRowHeight = 20;
    this.vTodayPx = -1;
    this.vLangs = lang;
    ...
    this.Draw = function() {
        ...
        if (this.vTaskList.length > 0) {
            ...
            let vColSpan = 1;
            let vTempJalali = moment(vTmpDate).locale('fa');
            let vMaxJalali = moment(vMaxDate).locale('fa');
            let vMinJalali = moment(vMinDate).locale('fa');
            // Major Date Header
            while (vTempJalali.isBefore(moment(vMaxJalali).locale('fa'))) {
                ...
                if (this.vFormat === 'day') {
                    ...
                    vCellContents += formatDateStr(vTempJalali.toDate(), this.vDayMajorDateDisplayFormat, this.vLangs[this.vLang]);
                    vTempJalali.day(vTempJalali.day() + 6);

                    if (this.vShowEndWeekDate === 1)
                        vCellContents +=
                            ' - ' + formatDateStr(vTempJalali.toDate(), this.vDayMajorDateDisplayFormat, this.vLangs[this.vLang]);

                    newNode(vTmpCell, 'div', null, null, vCellContents, vColWidth * colspan);

                    vTempJalali.day(vTempJalali.day() + 1);
                    ...
                    newNode(
                        ...
                        null,
                        null,
                        formatDateStr(vTempJalali.toDate(), this.vWeekMajorDateDisplayFormat, this.vLangs[this.vLang]),
                        vColWidth
                    );

                    vTempJalali.day(vTempJalali.day() + 7);
                } else if (this.vFormat === 'month') {
                    vColSpan = 12 - vTempJalali.month();
                    if (
                        vTempJalali.year() ===
                        moment(vMaxJalali)
                            .locale('fa')
                    ...
                    newNode(
                        ...
                        null,
                        null,
                        formatDateStr(vTempJalali.toDate(), this.vMonthMajorDateDisplayFormat, this.vLangs[this.vLang]),
                        vColWidth * vColSpan
                    );

                    vTempJalali.year(vTempJalali.year() + 1);
                    vTempJalali.month(0);
                    vTempJalali.day(1);
                } else if (this.vFormat === 'quarter') {
                    vColSpan = 4 - Math.floor(vTempJalali.month() / 3);
                    if (
                        vTempJalali.year() ===
                        moment(vMaxJalali)
                            .locale('fa')
                    ...
                    newNode(
                        ...
                        null,
                        null,
                        formatDateStr(vTempJalali.toDate(), this.vQuarterMajorDateDisplayFormat, this.vLangs[this.vLang]),
                        vColWidth * vColSpan
                    );
                    ...
                    newNode(
                        ...
                        null,
                        null,
                        formatDateStr(vTempJalali.toDate(), this.vHourMajorDateDisplayFormat, this.vLangs[this.vLang]),
                        vColWidth * vColSpan
                    );
                    vTempJalali.hours(0);
                    vTempJalali.day(vTempJalali.day() + 1);
            ...
            for (i = 0; i < this.vTaskList.length; i++) {
                ...
                if ((curTaskEnd.getTime() - curTaskEnd.getTimezoneOffset() * 60000) % 86400000 === 0)
                    curTaskEnd = new Date(
                        ...
                        curTaskEnd.getMonth(),
                        curTaskEnd.getDate() + 1,
                        curTaskEnd.getHours(),
                        curTaskEnd.getMinutes(),
                        curTaskEnd.getSeconds()
                ...
                if (curTaskPlanStart && curTaskPlanEnd) {
                    if ((curTaskPlanEnd.getTime() - curTaskPlanEnd.getTimezoneOffset() * 60000) % 86400000 === 0)
                        curTaskPlanEnd = new Date(
                            curTaskPlanEnd.getFullYear(),
                            curTaskPlanEnd.getMonth(),
                            curTaskPlanEnd.getDate() + 1,
                            curTaskPlanEnd.getHours(),
                            curTaskPlanEnd.getMinutes(),
                            curTaskPlanEnd.getSeconds()
                        ); // add 1 day here to simplify calculations below
