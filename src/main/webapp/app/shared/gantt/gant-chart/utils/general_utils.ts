...
export const formatDateStr = function(pDate, pDateFormatArr, pL) {
    ...
    const vYear2Str = pDate
        .getFullYear()
        .toString()
        .substring(2, 4);
    const vMonthStr = pDate.getMonth() + 1 + '';
    const vMonthArr = new Array(
        pL['january'],
        pL['february'],
        pL['march'],
        pL['april'],
    ...
    const vDayArr = new Array(pL['sunday'], pL['monday'], pL['tuesday'], pL['wednesday'], pL['thursday'], pL['friday'], pL['saturday']);
    const vMthArr = new Array(
        pL['jan'],
        pL['feb'],
        pL['mar'],
        pL['apr'],
    ...
    const vDyArr = new Array(pL['sun'], pL['mon'], pL['tue'], pL['wed'], pL['thu'], pL['fri'], pL['sat']);
    ...
    for (let i = 0; i < pDateFormatArr.length; i++) {
        switch (pDateFormatArr[i]) {
            ...
            case 'week':
                ...
                const vDayOfWeek = pDate.getDay() == 0 ? 7 : pDate.getDay();
...
export const stripIds = function(pNode) {
    for (let i = 0; i < pNode.childNodes.length; i++) {
        ...
        if (pNode.childNodes[i].hasChildNodes()) {
            stripIds(pNode.childNodes[i]);
        }
    }
};

export const stripUnwanted = function(pNode) {
    const vAllowedTags = new Array('#text', 'p', 'br', 'ul', 'ol', 'li', 'div', 'span', 'img');
    for (let i = 0; i < pNode.childNodes.length; i++) {
        ...
        if (pNode.childNodes[i].hasChildNodes()) {
            stripUnwanted(pNode.childNodes[i]);
        }
    }
};

export const delayedHide = function(pGanttChartObj, pTool, pTimer) {
    const vDelay = pGanttChartObj.getTooltipDelay() || 1500;
    if (pTool) {
...
export const getZoomFactor = function() {
    ...
    if (document.body.getBoundingClientRect) {
        ...
        const vPhysicalW = vRect.right - vRect.left;
        const vLogicalW = document.body.offsetWidth;
...
export const benchMark = function(pItem) {
    const vEndTime = new Date().getTime();
...
export const getScrollPositions = function() {
    ...
    if (!('pageXOffset' in window)) {
        const vZoomFactor = getZoomFactor();
...
export const getOffset = function(pStartDate, pEndDate, pColWidth, pFormat, pShowWeekends) {
    ...
    const vMonthDaysArr = new Array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    const curTaskStart = new Date(pStartDate.getTime());
    const curTaskEnd = new Date(pEndDate.getTime());
    let vTaskRightPx = 0;
    const tmpTaskStart = Date.UTC(
    ...
    const tmpTaskEnd = Date.UTC(curTaskEnd.getFullYear(), curTaskEnd.getMonth(), curTaskEnd.getDate(), curTaskEnd.getHours(), 0, 0);
    ...
    if (pFormat == 'day') {
        ...
        const vMonthsDiff =
            12 * (curTaskEnd.getFullYear() - curTaskStart.getFullYear()) + (curTaskEnd.getMonth() - curTaskStart.getMonth());
        vPosTmpDate = new Date(curTaskEnd.getTime());
        vPosTmpDate.setDate(curTaskStart.getDate());
        const vDaysCrctn = (curTaskEnd.getTime() - vPosTmpDate.getTime()) / 86400000;

        vTaskRightPx = Math.ceil(
            vMonthsDiff * (pColWidth + MONTH_CELL_MARGIN_WIDTH) + vDaysCrctn * (pColWidth / vMonthDaysArr[curTaskEnd.getMonth()]) - 1
        );
    } else if (pFormat == 'quarter') {
        const vMonthsDiff =
            12 * (curTaskEnd.getFullYear() - curTaskStart.getFullYear()) + (curTaskEnd.getMonth() - curTaskStart.getMonth());
        ...
        const vMinsCrctn = (curTaskEnd.getTime() - vPosTmpDate.getTime()) / 3600000;
...
export const fadeToolTip = function(pDirection, pTool, pMaxAlpha) {
    const vIncrement = parseInt(pTool.getAttribute('fadeIncrement'));
    let vAlpha = pTool.getAttribute('currentOpacity');
    const vCurAlpha = parseInt(vAlpha);
...
export const updateFlyingObj = function(e, pGanttChartObj, pTimer) {
    ...
    const vCurLeftBuf = 5;
    const vCurBotBuf = 3;
    const vCurRightBuf = 15;
    let vMouseX = e ? e.clientX : (<MouseEvent>window.event).clientX;
    let vMouseY = e ? e.clientY : (<MouseEvent>window.event).clientY;
    const vViewportX = document.documentElement.clientWidth || document.getElementsByTagName('body')[0].clientWidth;
    const vViewportY = document.documentElement.clientHeight || document.getElementsByTagName('body')[0].clientHeight;
    let vNewX = vMouseX;
    let vNewY = vMouseY;
    ...
    const vScrollPos = getScrollPositions();
...
export const moveToolTip = function(pNewX, pNewY, pTool, timer) {
    const vSpeed = parseInt(pTool.getAttribute('moveSpeed'));
