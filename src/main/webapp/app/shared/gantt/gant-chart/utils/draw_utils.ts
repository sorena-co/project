        ...
        switch (type) {
            ...
            case 'resource':
                if (choices) {
                    ...
                    return (
                        `<select>` +
                        choices.map(c => `<option value="${c.id}" ${value === c.id ? 'selected' : ''} >${c.name}</option>`).join('') +
                        `</select>`
                    );
                } else {
...
export const drawSelector = function(pPos) {
    const vOutput = document.createDocumentFragment();
    let vDisplay = false;
    ...
    if (vDisplay) {
        const vTmpDiv = newNode(vOutput, 'div', null, 'gselector', this.vLangs[this.vLang]['format'] + ':');
