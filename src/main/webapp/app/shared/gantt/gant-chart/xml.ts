...
export const AddXMLTask = function(pGanttVar, pXmlDoc) {
    ...
    let maxPID = 0;
    const ass = new Array();
    const assRes = new Array();
    const res = new Array();
    const pars = new Array();

    const projNode = findXMLNode(pXmlDoc, 'Project');
    ...
    if (project == 'http://schemas.microsoft.com/project') {
        ...
        const resources = findXMLNode(pXmlDoc, 'Resource');
        ...
        for (i = 0; i < m; i++) {
            const resname = getXMLNodeValue(resources[i], 'Name', 2, '');
            const uid = getXMLNodeValue(resources[i], 'UID', 1, -1);
        ...
        const assignments = findXMLNode(pXmlDoc, 'Assignment');
        ...
        for (i = 0; i < j; i++) {
            let uid;
            const resUID = getXMLNodeValue(assignments[i], 'ResourceUID', 1, -1);
        ...
        for (i = 0; i < n; i++) {
            ...
            const pID = getXMLNodeValue(Task[i], 'UID', 1, 0);

            if (pID != 0) {
                const pName = getXMLNodeValue(Task[i], 'Name', 2, 'No Task Name');
                const pStart = getXMLNodeValue(Task[i], 'Start', 2, '');
                const pEnd = getXMLNodeValue(Task[i], 'Finish', 2, '');
                const pLink = getXMLNodeValue(Task[i], 'HyperlinkAddress', 2, '');
                const pMile = getXMLNodeValue(Task[i], 'Milestone', 1, 0);
                const pComp = getXMLNodeValue(Task[i], 'PercentWorkComplete', 1, 0);
                const pCost = getXMLNodeValue(Task[i], 'Cost', 2, 0);
                let pGroup = getXMLNodeValue(Task[i], 'Summary', 1, 0);

                let pParent = 0;

                const vOutlineLevel = getXMLNodeValue(Task[i], 'OutlineLevel', 1, 0);
                let vOutlineNumber;
                if (vOutlineLevel > 1) {
                    vOutlineNumber = getXMLNodeValue(Task[i], 'OutlineNumber', 2, '0');
                ...
                const predecessors = findXMLNode(Task[i], 'PredecessorLink');
                ...
                for (k = 0; k < j; k++) {
                    const depUID = getXMLNodeValue(predecessors[k], 'PredecessorUID', 1, -1);
                    const depType = getXMLNodeValue(predecessors[k], 'Type', 1, 1);
                ...
                const splits = findXMLNode(ass[pID], 'TimephasedData');
        ...
        for (i = 0; i < n; i++) {
            // optional parameters may not have an entry
            // Task ID must NOT be zero otherwise it will be skipped
            const pID = getXMLNodeValue(Task[i], 'pID', 1, 0);

            if (pID != 0) {
                const pName = getXMLNodeValue(Task[i], 'pName', 2, 'No Task Name');
                const pStart = getXMLNodeValue(Task[i], 'pStart', 2, '');
                const pEnd = getXMLNodeValue(Task[i], 'pEnd', 2, '');
                const pLink = getXMLNodeValue(Task[i], 'pLink', 2, '');
                const pMile = getXMLNodeValue(Task[i], 'pMile', 1, 0);
                const pComp = getXMLNodeValue(Task[i], 'pComp', 1, 0);
                const pCost = getXMLNodeValue(Task[i], 'pCost', 2, 0);
                const pGroup = getXMLNodeValue(Task[i], 'pGroup', 1, 0);
                const pParent = getXMLNodeValue(Task[i], 'pParent', 1, 0);
                const pRes = getXMLNodeValue(Task[i], 'pRes', 2, '');
                const pOpen = getXMLNodeValue(Task[i], 'pOpen', 1, 1);
                const pDepend = getXMLNodeValue(Task[i], 'pDepend', 2, '');
                const pCaption = getXMLNodeValue(Task[i], 'pCaption', 2, '');
                const pNotes = getXMLNodeValue(Task[i], 'pNotes', 2, '');
...
export const getXMLTask = function(pID, pIdx) {
    ...
    let vIdx = -1;
    let vTask = '';
    const vOutFrmt = parseDateFormatStr(this.getDateInputFormat() + ' HH:MI');
    if (pIdx === true) {
        vIdx = pID;
    } else {
    ...
    if (vIdx >= 0 && vIdx < this.vTaskList.length) {
        ...
        vTask += '<pDepend>';
        const vDepList = this.vTaskList[vIdx].getDepend();
        ...
        const vTmpFrag = document.createDocumentFragment();
        const vTmpDiv = newNode(vTmpFrag, 'div', null, null, this.vTaskList[vIdx].getNotes().innerHTML);
