...
export const TaskItem = function(
    ...
    const vGantt = pGantt ? pGantt : this;
    const _id = document.createTextNode(pID).data;
    const vID = hashKey(document.createTextNode(pID).data);
    ...
    const vLink = document.createTextNode(pLink).data;
    const vMile = parseInt(document.createTextNode(pMile).data);
    ...
    const vParent = parent;

    let vOpen = vGroup == 2 ? 1 : parseInt(document.createTextNode(pOpen).data);
    const vDepend = new Array();
    const vDependType = new Array();
    const vCaption = document.createTextNode(pCaption).data;
    ...
    if (pPlanStart != null && pPlanStart != '') {
        vPlanStart =
            pPlanStart instanceof Date ? pPlanStart : parseDateStr(document.createTextNode(pPlanStart).data, vGantt.getDateInputFormat());
    ...
    if (pDepend != null) {
        const vDependStr = pDepend + '';
        const vDepList = vDependStr.split(',');
        const n = vDepList.length;
...
export const createTaskInfo = function(pTask, templateStrOrFn = null) {
    let vTmpDiv;
    const vTaskInfoBox = document.createDocumentFragment();
    const vTaskInfo = newNode(vTaskInfoBox, 'div', null, 'gTaskInfo');

    const setupTemplate = template => {
        vTaskInfo.innerHTML = '';
        if (template) {
            const allData = pTask.getAllData();
