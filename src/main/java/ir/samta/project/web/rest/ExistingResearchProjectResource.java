...
public class ExistingResearchProjectResource {
    ...
    /**
     * SEARCH  /_search/existing-research-projects?query=:query : search for the existingResearchProject corresponding
     * to the query.
     *
     * @param query the query of the existingResearchProject search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/existing-research-projects")
    public ResponseEntity<List<ExistingResearchProjectDTO>> searchExistingResearchProjects(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ExistingResearchProjects for query {}", query);
        Page<ExistingResearchProjectDTO> page = existingResearchProjectService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/existing-research-projects");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
