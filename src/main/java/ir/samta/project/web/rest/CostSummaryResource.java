...
public class CostSummaryResource {
    ...
    /**
     * SEARCH  /_search/cost-summaries?query=:query : search for the costSummary corresponding
     * to the query.
     *
     * @param query the query of the costSummary search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/cost-summaries")
    public ResponseEntity<List<CostSummaryDTO>> searchCostSummaries(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of CostSummaries for query {}", query);
        Page<CostSummaryDTO> page = costSummaryService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/cost-summaries");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
